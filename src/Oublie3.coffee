{__, all, always, any, assoc, call, compose, contains, difference, either, filter, flip, fromPairs, has, isEmpty, isNil, keys, lt, map, match, merge, once, pickBy, reject, remove, replace, set, split, take, test, type, union, uniq} = require 'ramda' #auto_require:ramda
...
class Oublie
	...
	oneOrMany: (key, query, strategy, expiry) ->
		...
		remoteQuery = calcRemoteQuery @data, query, strategy
	...
	#		- rename to removeExpired
	#		- removeExpired borde returnera en spec
	#		- kolla om changedPaths klarar $assoc
	...
	__commitAndPublish: ->
		...
		affectedSubs = calcAffectedSubs delta, @state
	...
	runSub: (state, entity, key) ->
		...
		if hashResult != sub.lastResult
			...
			@change {subs: {"#{entity}": {"#{key}": {lastResult: hashResult}}}}
...
calcRemoteQuery = (data, query, strategy) ->
	switch strategy
		when 'LO' then null
		when 'PE' then query
		when 'OP'
			if isCached(data, query) == true then null
			else query
		when 'VO'
			cacheResult = isCached data, query
...
calcChangedSubs = (paths) ->
	# change of lastResult is not a real sub change
